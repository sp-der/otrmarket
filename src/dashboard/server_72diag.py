...
def _audit_loss_context() -> None:
    ...
    try:
        rows = connection.execute(
            ...
            """.format(",".join("?" for _ in LOSS_IDS)),
            tuple(sorted(LOSS_IDS)),
        ).fetchall()
        print(f"LOSS CONTEXT AUDIT 7.2 rows={len(rows)}", flush=True)
        for row in rows:
            try:
            ...
            print(
                "LOSS CONTEXT 7.2 " + repr({
                    ...
                    "quality_grade": ctx.get("quality_grade"),
                    "quality_score": ctx.get("quality_score"),
                    "context_tf": ctx.get("context_timeframe"),
                    "context_bias": ctx.get("higher_timeframe_bias"),
                    "narrative_tf": ctx.get("narrative_timeframe"),
                    "narrative_bias": ctx.get("narrative_bias"),
                    "narrative_conflict": ctx.get("narrative_conflict"),
                    "disp_body": ctx.get("displacement_body_ratio"),
                    "disp_range": ctx.get("displacement_range_ratio"),
                    "fvg_age": ctx.get("entry_fvg_age_bars"),
                    "mi_score": ctx.get("market_intelligence_score"),
