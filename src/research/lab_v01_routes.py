...
.grid{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:10px;margin-bottom:22px}.card{border:1px solid #252525;background:#111;border-radius:14px;padding:14px}.label{font-size:11px;letter-spacing:.08em;text-transform:uppercase;color:#888}.value{font-size:25px;font-weight:850;margin-top:5px}.small{font-size:12px;color:#888;margin-top:3px}
...
@media(max-width:1000px){.grid{grid-template-columns:repeat(3,minmax(0,1fr))}}@media(max-width:620px){.grid{grid-template-columns:repeat(2,minmax(0,1fr))}table{display:block;overflow-x:auto}.value{font-size:21px}}
</style>
</head>
<body><main>
...
<div class="card"><div class="label">Auto Certifier</div><div class="value" id="auto">STARTING</div><div class="small" id="autocount">0 certified</div></div>
</div>
...
function paint(d){
 const b=d.baseline||{},a=d.auto_certification||{},counts=a.counts||{};
 $('samples').textContent=b.samples??0;$('progress').textContent=`${b.samples??0} / ${b.minimum_samples??20}`;
 $('wl').textContent=`${b.wins??0} / ${b.losses??0}`;$('wr').textContent='Win rate '+pct(b.win_rate);
 $('pnl').textContent=money(b.net_pnl);$('expr').textContent=b.expectancy_r==null?'n/a':Number(b.expectancy_r).toFixed(2)+'R';$('dd').textContent=money(b.max_drawdown);
 $('parity').textContent=`${b.parity_path_matches??0} / ${b.parity_samples??0}`;$('evidence').textContent=b.evidence_status||'NOT_EVALUABLE';
 $('auto').textContent=a.worker_running?'RUNNING':a.enabled?'STARTING':'OFF';
 $('autocount').textContent=`${counts.CERTIFIED||0} certified · ${counts.UNAVAILABLE_RETENTION||0} old unavailable`;
...
def install_research_lab_routes() -> None:
    """Register protected Research Lab surfaces and start the read-only certifier."""
    ...
    # The worker lives in the dashboard/research process, not the strategy
    # process. A Nautilus failure therefore cannot block market processing,
    # setup approval, paper fills, broker handoff, or risk management.
    start_auto_certifier()
